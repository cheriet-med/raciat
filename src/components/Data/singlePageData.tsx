      ...
      <div className="container mx-auto  py-6">
        <div className="flex flex-col lg:flex-row gap-2">
          ...
          <div className="flex-1 space-y-2">
            ...
            <Card>
              <CardHeader>
                <CardTitle className="font-playfair">التفاصيل</CardTitle>

              </CardHeader>
              <CardContent className="space-y-6">
                <div className="flex gap-2 flex-wrap">

                  {info.organic_ingredients?  <p className="py-1.5 px-4 bg-yel text-white font-semibold w-fit rounded-xl">Organic Ingredients</p> : ""}
                  {info.organic_ingredients?    <p className="py-1.5 px-4 bg-yel text-white font-semibold w-fit rounded-xl">Sustainable Seafood</p> : ""}
                 
                </div>
              
                <div className="grid grid-cols-2 gap-6">
                  <div>
                    <h4 className="font-semibold mb-3 font-playfair">وسائل الراحة في العقار</h4>
                  ...
                  <div>
                    <h4 className="font-semibold mb-3 font-playfair">مميزات أخرى</h4>
                    <div className="grid grid-cols-1 gap-2">
...
{info.category == "Restaurant" ?
  <div>
                    <h4 className="font-semibold mb-4 font-playfair">ساعات المعاينة</h4>
...
:""}
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    {info.category == "Restaurant" ?  <h4 className="font-semibold mb-2 font-playfair">نوع العقار</h4> : <h4 className="font-semibold mb-2 font-playfair">Room type</h4> }
                   ...
                   <div className="flex gap-2 flex-wrap  mt-4">
                    ...
                    <div className="text-sm text-white bg-yel w-fit py-1 px-4 rounded-3xl capitalize font-bold  flex gap-2 items-center" >
                    <GiCalendarHalfYear className="w-5 h-5"/>
                    <p>تاريخ اﻹنشاء - {info.established}</p>
                   </div>

                  <div className="text-sm text-white bg-yel w-fit py-1 px-4 rounded-3xl capitalize font-bold  flex gap-2 items-center" >
                    <SlSizeFullscreen className="w-5 h-5"/>
                    <p>المساحة - {info.size}</p>
                   </div>

                  <div className="text-sm text-white bg-yel w-fit py-1 px-4 rounded-3xl capitalize font-bold  flex gap-2 items-center" >
                    <SiMyspace className="w-5 h-5"/>
                    <p>السعة - {info.capacity}</p>
                   </div>
  ...
  <Card>
          ...
          <div >
            ...
            <Card>
              <CardHeader>
                <CardTitle className="font-playfair">تفاصيل التقييم</CardTitle>
                <div className="flex items-center gap-2 flex-wrap">
                  <span className="text-2xl font-bold text-gray-900">{averageRating}</span>
                  <StarRating rating={averageRating} size={16}/>
                   <span className="text-accent font-medium">{averageRating == 5? "Excellent": (averageRating == 4? "Very Good" :(averageRating == 3? "Good":(averageRating == 2? 	"Poor" : "")))}</span>
                  <span className="text-sm text-gray-500">({Review.length} تقييمات)</span>
                </div>
              </CardHeader>
              <CardContent>
                <div className="space-y-4">
                  ...
                  <div className="grid gap-3 md:w-[400px]">
                    {[
                      { label: "الموقع", score: Review && Review.length > 0? Review.reduce((sum, r) => sum + +r.location, 0) / Review.length: 0, color: "bg-accent" },
                      { label: "الغرف", score: Review && Review.length > 0? Review.reduce((sum, r) => sum + +r.room, 0) / Review.length: 0, color: "bg-accent" },
                      { label: "القيمة", score: Review && Review.length > 0? Review.reduce((sum, r) => sum + +r.value, 0) / Review.length: 0, color: "bg-accent" },
                      { label: "النضافة", score: Review && Review.length > 0? Review.reduce((sum, r) => sum + +r.clearliness, 0) / Review.length: 0, color: "bg-accent" },
                      { label: "الخدمة", score: Review && Review.length > 0? Review.reduce((sum, r) => sum + +r.service, 0) / Review.length: 0, color: "bg-accent" },
                      { label: "المساحة", score: Review && Review.length > 0? Review.reduce((sum, r) => sum + +r.restaurant_space, 0) / Review.length: 0, color: "bg-accent" }
                    ].map((item) => (
                      <div key={item.label} className="flex items-center gap-4">
                        <div className="w-24 md:w-28  text-sm font-medium text-gray-500">{item.label}</div>
                        <div className="flex-1 bg-gray-100 rounded-full h-4">
...
{/**Booking Form */}
<div id="book">
     <RestaurantBookingComponent bookdata={info}/>
</div>
          </div>
        </div>
      </div>
